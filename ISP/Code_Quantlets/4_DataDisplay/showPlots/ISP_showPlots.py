...
- grouped boxplots
- pieplots
- scatterplots, with markersize proportional to the value of
...
def printout(outFile, xlabel = '', ylabel='', title='', outDir = '.') -> None:
...
def simplePlots() -> None:
    ...
    # Scatter plot
    plt.scatter(np.arange(len(x)), x, s=1)
    plt.xlim([0, len(x)])
    
    # Save and show the data, in a systematic format
    printout('scatterPlot.png', xlabel='Datapoints', ylabel='Values',
             title='Scatter')
    ...
    plt.xlim([-0.2, 4.2])
    plt.ylim([-0.2, 19])
    printout('Errorbars.png', xlabel='Data Values', ylabel='Measurements',
             title='Errorbars')
