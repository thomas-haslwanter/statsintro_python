...
def fisherExact() -> float:
    """Fisher's Exact Test:
    Data are taken from Altman, Table 10.14
    Spectacle wearing among juvenile delinquensts and non-delinquents who
    failed a vision test
    Spectecle wearers: 1 delinquent, 5 non-delinquents
    Non-spectacle wearers: 8 delinquents, 2 non-delinquents

    Returns
    -------
    fisher_result : p-value for Fisher's Exact Test
    """
    ...
    'hypothesis is true, is p={0:5.3f}.'.format(fisher_result[1])))
...
def tryMcnemar() -> None:
    ...
    approximate formulas are typically given in the lecture scripts. Just
    ignore the statistic that is returned, because it is different for the two
    options.
    ...
    The test requires the same subjects to be included in the before-and-after
